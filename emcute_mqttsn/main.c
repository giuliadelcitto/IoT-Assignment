...
int data_gen(char* data_sens, int i)
{
	static int data[5];
	...
	data[4]= rand() % 51;
	if(i%2==0){
		sprintf(data_sens , "1,%d,%d,%d,%d,%d", data[0],data[1],data[2],data[3],data[4]);
	}
	else{
		sprintf(data_sens , "2,%d,%d,%d,%d,%d", data[0],data[1],data[2],data[3],data[4]);
	}
	
	return 0;
}

static int cmd_pub_sensor_data(int argc, char **argv)
{   
	argv[1]="sensor/data";
    emcute_topic_t s1;
    ...
    while(1){
		...
		if (emcute_reg(&s1) != EMCUTE_OK) {
			puts("error: unable to obtain topic ID");
			return 1;
		}

		/* step 2: publish data */
		if (emcute_pub(&s1, data_s1, strlen(data_s1), flags) != EMCUTE_OK) {
			printf("error: unable to publish data to topic '%s [%i]'\n",
					s1.name, (int)s1.id);
			return 1;
		}
		...
		printf("Published %i bytes to topic '%s [%i]'\n",
				(int)strlen(data_s1), s1.name, s1.id);
		xtimer_sleep(6);
    }
...
static const shell_command_t shell_commands[] = {
    ...
    { "pub_sensor_data", "publish sensor data ", cmd_pub_sensor_data},
    { NULL, NULL, NULL }
};
